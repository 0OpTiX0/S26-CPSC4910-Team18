...
@app.post("/redeem")
def redeemItem(payload: RedeemRequest, session: Session = Depends(getSession)):
    if payload.quantity <= 0:
        raise HTTPException(status_code=400, detail="Quantity must be greater than 0")

    driver = session.exec(
        select(Driver_User).where(Driver_User.UserID == payload.driver_id)
    ).first()
    ...
    product = session.exec(
        select(Product).where(Product.ProductID == payload.product_id)
    ).first()

    if not product:
        raise HTTPException(status_code=404, detail="Product not found")

    if product.ProductQty < payload.quantity:
        raise HTTPException(status_code=400, detail="Not enough inventory available")

    total_cost = product.ProductPrice * payload.quantity
    ...
    driver.User_Points -= total_cost

    # reduce product inventory
    product.ProductQty -= payload.quantity
    product.LastRefreshed = datetime.now(timezone.utc)
    ...
    user = session.exec(
        select(User).where(User.UserID == payload.driver_id)
    ).first()

    sponsor = None
    if driver.Sponsor_ID is not None:
        sponsor = session.exec(
            select(Sponsor).where(Sponsor.Sponsor_ID == driver.Sponsor_ID)
        ).first()

    transaction = Point_Transaction(
        Driver_User_ID=payload.driver_id,
        Driver_Name=user.User_Name if user else "Unknown Driver",
        Sponsor_Name=sponsor.Sponsor_Name if sponsor else "No Sponsor",
        Points_Change=str(-total_cost),
        Points_After_Change=driver.User_Points,
        Reason_For_Change=f"Redeemed {payload.quantity} x {product.ProductName}",
        Created_At=datetime.now(timezone.utc)
    )
    ...
    return {
        ...
        "remaining_points": driver.User_Points
    }
